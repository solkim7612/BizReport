...
function CitReport({ onQueueUpdated }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await apiClient.get(`/reports/get/${loginId}?reportType=CIT&endMon=${selectedMon}`);
      ...
      const targetYear = parseInt(selectedMon.split('-')[0], 10);
      const targetMonth = selectedMon.split('-')[1];
      const years = [targetYear - 2, targetYear - 1, targetYear];

      const promises = years.map(y => {
        const monthToFetch = (y === targetYear) ? targetMonth : '12';
        return apiClient.get(`/reports/get/${loginId}?reportType=CIT&endMon=${y}-${monthToFetch}`).catch(() => null);
      });

      const results = await Promise.all(promises);

      const newTrendData = results.map((res, idx) => {
        const acc = res?.data?.find(r => r.periodType === 'ACCUMULATED');
        return {
          name: `${years[idx]}년`,
          sales: Math.floor((acc?.calc?.totalSales || 0) / 10000),
          purchase: Math.floor((acc?.calc?.totalPurchase || 0) / 10000),
          tax: Math.floor((acc?.calc?.beforeTax || 0) / 10000)
        };
      });
  ...
  const fetchPrepaidTax = async () => {
    try {
      const response = await apiClient.get(`/reports/prepaid-tax/${loginId}`, {
        params: { reportType: 'CIT', endMon: selectedMon }
      });
  ...
  const renderCitDetail = (report) => {
    ...
    return (
      <div className="calc-detail-box">
        ...
        <div className="calc-row"><span>총 수입금액 (+)</span><span className="text-red">{c.totalSales?.toLocaleString()} 원</span></div>
        <div className="calc-row"><span>필요경비 (-)</span><span className="text-blue">{c.totalPurchase?.toLocaleString()} 원</span></div>
        <div className="calc-row fw-bold"><span>종합소득금액</span><span>{c.profit?.toLocaleString()} 원</span></div>
        <hr/>
        ...
        <div className="calc-total-row">
          <span>{c.isRefund ? '환급 받을 세액' : '최종 납부 세액'}</span>
        ...
        <div className="graph-comparison">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={trendData} margin={{ top: 10, right: 10, bottom: 0, left: -20 }}>
              ...
              <Bar dataKey="sales" name="수입금액" barSize={35} fill="#ef4444" radius={[4, 4, 0, 0]} />
              <Bar dataKey="purchase" name="필요경비" barSize={35} fill="#3b82f6" radius={[4, 4, 0, 0]} />
  ...
  const year = selectedMon.split('-')[0];
        ...
        <div className="report-card">
          <div className="report-card-header" onClick={() => !isAccNotFound && setIsAccOpen(!isAccOpen)}>
            <div className="report-title">
              ...
              <strong>{year}년 전체</strong> 예상 {accReport?.calc?.isRefund ? '환급' : '납부'}세액
              {isAccNotFound ? (
                <span className="highlight-tax text-gray"> - 원</span>
              ) : (
