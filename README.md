...
| Batch | 사용자별 배치 대기열 조회        | GET    | /api/v1/batch/queue/{id}              | 카드 업로드 배치 진행 현황                    |
| Batch | 배치 파일 데이터 미리보기        | GET    | /api/v1/batch/queue/detail/{batchId}  | 대기열에 등록된 카드 내역 데이터 확인              |
| Batch | 전체 배치 대기열 조회          | GET    | /api/v1/batch/queue                   |                         |
...
- Phase 2. 원본 데이터 직접 삭제 및 삽입 (이슈 발견)
...
### 8.3. Producer-Consumer 기반 배치 큐 전환
대기열 기반 비동기 파이프라인으로 전환하는 과정에서 발생한 동시성 및 트랜잭션 충돌을 해결하기 위해 아키텍처를 3단계에 걸쳐 고도화

- Phase 1. 스케줄러의 Job 직접 즉시 실행 (도입 보류)
  - 접근: 특정 스케줄 시간대에 도달하면 스케줄러 스레드가 JobLauncher.run()을 직접 호출하여 즉시 배치를 실행
  - 한계: 무거운 배치 작업이 스케줄러 스레드를 장시간 점유하여 스케줄 지연 및 리소스 고갈 유발
- Phase 2. DB 기반 큐 도입 및 Multi-Producer / Multi-Consumer 병렬 실행 (이슈 발견)
  - 접근: 스케줄러 및 API 요청은 배치 작업을 READY 상태로 큐에 적재만 하고, 다중 컨슈머 스레드로 병렬 실행 시도 
  - 한계: 여러 컨슈머 스레드가 동시에 jobLauncher.run()을 호출하면서 Spring Batch 메타 테이블에 동시 INSERT/UPDATE가 발생하여 DB Deadlock 발생
- Phase 3. Single-Consumer Polling (최종 해결)
  - 해결: 락 충돌 및 데드락을 차단하기 위해 Multi-Producer / Single-Consumer Polling 구조 도입
    1. Multi-Producer 유지
    2. 등록된 READY 작업들을 60초 주기로 폴링하여, 단일 워커 흐름에서 순차적으로 JobLauncher를 실행해서 메타 테이블 락 충돌 및 데드락 원천 차단
    3. 큐 상태 업데이트(PROCESSING, COMPLETED, FAILED)만 개별 트랜잭션으로 격리
  - 성과: 스케줄러 스레드 블로킹 해소, 락 충돌 없는 무결한 배치 실행 환경 완성
